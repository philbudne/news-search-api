...
# NOTE! asynctest appears to be unmaintained, and Python 3.10 spews
# warnings about its use of deprecated @asyncio.coroutine construct,
# BUT pytest-asyncio required MAJOR changes to this file (no TestCase
# class), and would not run all tests!
import asynctest
...
class ApiTest(asynctest.TestCase):
    ...
    async def test_api_version_response_header(self):
        response = await self._client.get(f'/', timeout=TIMEOUT)
        assert response.status_code == 200
        assert 'x-api-version' in response.headers
        assert response.headers['x-api-version'] == api.ApiVersion.v1

    async def test_overview_all(self):
        # make sure all stories come back and domain is right
        response = await self._client.post(f'/v1/{INDEX_NAME}/search/overview', json={"q": "*"}, timeout=TIMEOUT)
        assert response.status_code == 200
    ...
    async def test_overview_no_results(self):
        response = await self._client.post(f'/v1/{INDEX_NAME}/search/overview', json={"q": "asdfdf"}, timeout=TIMEOUT)
        assert response.status_code == 404

    async def test_overview_by_content(self):
        response = await self._client.post(f'/v1/{INDEX_NAME}/search/overview', json={"q": "article"}, timeout=TIMEOUT)
        assert response.status_code == 200
        results = response.json()
        assert 'total' in results
        assert results['total'] > 1000
        response = await self._client.post(f'/v1/{INDEX_NAME}/search/overview', json={"q": "1"}, timeout=TIMEOUT)
        assert response.status_code == 200
        results = response.json()
        assert 'total' in results
        assert results['total'] < 1000

    async def test_overview_by_pub_date(self):
        response = await self._client.post(f'/v1/{INDEX_NAME}/search/overview',
    ...
    async def test_paging_token(self):
        response = await self._client.post(f'/v1/{INDEX_NAME}/search/result',
    ...
    async def _get_all_stories(self, page_size: int, query: str = "*"):
        more_stories = True
    ...
    async def test_paging_all(self):
        # get total count
        response = await self._client.post(f'/v1/{INDEX_NAME}/search/overview', json={"q": "*"}, timeout=TIMEOUT)
        assert response.status_code == 200
        results = response.json()
        ...
        page_size = 1000
        page_count, stories = await self._get_all_stories(page_size)
    ...
    async def test_text_content_expanded(self):
        response = await self._client.post(f'/v1/{INDEX_NAME}/search/result',
    ...
    async def test_story_sort_order(self):
        # desc
        response = await self._client.post(f'/v1/{INDEX_NAME}/search/result',
                                           json={"q": "*"}, timeout=TIMEOUT)
        assert response.status_code == 200
    ...
    async def test_story_sort_field(self):
        # desc
        response = await self._client.post(f'/v1/{INDEX_NAME}/search/result',
    ...
    async def test_date_formats(self):
        response = await self._client.post(f'/v1/{INDEX_NAME}/search/result', json={"q": "*"}, timeout=TIMEOUT)
        assert response.status_code == 200
        results = response.json()
        for story in results:
    ...
    async def test_filter_by_indexed_date(self):
        response = await self._client.post(f'/v1/{INDEX_NAME}/search/result',
    ...
    async def test_sort_duplicate_pub_date(self):
        small_page_size = 20
        query = "* AND publication_date:[2023-12-01 TO 2023-12-05]"
        # check enough stories
        response = await self._client.post(f'/v1/{INDEX_NAME}/search/overview', json={"q": query}, timeout=TIMEOUT)
        assert response.status_code == 200
        results = response.json()
        assert 'total' in results
        assert results['total'] > small_page_size * 2
        # now pull the same results twice to compare
        _, stories1 = await self._get_all_stories(small_page_size, query)
        url_list_take1 = [s['normalized_url'] for s in stories1]
        _, stories2 = await self._get_all_stories(small_page_size, query)
